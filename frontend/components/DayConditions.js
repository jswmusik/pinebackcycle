...
// bare=true: rendera utan eget kort/rubrik (används i modal som redan har titel).
export default function DayConditions({ dayId, routeKey, bare = false }) {
  ...
  // Vilodag eller fel → visa ingenting.
  if (!loading && (!data || (!data.available && data.reason === "rest_day"))) {
    return null;
  }

  const heading = !bare && <h3 style={{ marginTop: 0 }}>Dagens förutsättningar</h3>;
  ...
  return bare ? body : <div className="card conditions-card">{body}</div>;
}
